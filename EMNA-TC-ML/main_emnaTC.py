from emna_tc import emna_tc # replaced
...
for run in range(runs):
    for fun_num in range(1, 29):
        threshold, best_f = emna_tc(obj_function, dim, max_evals, popsize, 100)
        if run == 0:
...
pickle.dump(fits, open("pickles/results_EMNA_TC_fits.p", "wb"), protocol=4)
pickle.dump(thresholds, open("pickles/results_EMNA_TC_thresholds.p", "wb"), protocol=4)
print('fitnesses:')
print(fits)