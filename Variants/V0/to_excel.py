...
init_file = '../abr data/emna_g_exp' # Replace with file name that contains the data
second_file = '/fun'
mid_file = '_exp'
end_file_name = '.p'
exp_dir_nums = []
...
titles3 = ['exp', 'func', 'number used', 'number excluded']

current_dir = os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))
parent_dir = os.path.dirname(current_dir)
sys.path.insert(0,parent_dir)

workbook = xlsxwriter.Workbook('data_col.xlsx') # First Excel file
workbook.use_zip64()
worksheet = workbook.add_worksheet('main data')
workbook2 = xlsxwriter.Workbook('excel_files/data_infor.xlsx') # Info Excel file
...
for e in range(1,len(exp_dir_nums) + 1):
    ...
    for i in range(start_file, functions[e - 1] + 1):
        ...
        #filename = init_file + str(i) + ending_file
        expString = ''
        if e < 9:
        ...
        for j in range(0, len(exampleContents)):
            #for k in range(0, len(exampleContents[j].data[0])):
...
switchFile = pd.read_excel(r'data_col.xlsx')
switchFile.to_csv(r'excel_files/data_col2.csv', index=None, header=True)

# Remove unnecessary file
os.remove('data_col.xlsx')